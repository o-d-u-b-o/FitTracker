...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	return ((runningCaloriesMeanSpeedMultiplier * runSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	walkingSpeedKm := meanSpeed(action, duration)
	walkingSpeedMs := walkingSpeedKm * kmhInMsec
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(walkingSpeedMs, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	SwimmingSpentCalories := (swimmingSpeed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
